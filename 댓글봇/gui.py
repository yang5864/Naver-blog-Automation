...
class App(ctk.CTk):
    def __init__(self, config: AppConfig):
        ...
        ctk.CTkLabel(
            self.frame_search, text="댓글 페르소나", font=IOS_FONT_MEDIUM, text_color=IOS_COLORS["text_primary"]
        ).pack(anchor="w", padx=20, pady=(20, 14))

        ctk.CTkLabel(
            self.frame_search,
            text="예: 30대 마케터, 밝고 예의있는 말투, IT/생산성 관심",
            font=IOS_FONT_SMALL,
            text_color=IOS_COLORS["text_secondary"],
        ).pack(anchor="w", padx=20, pady=(0, 8))

        self.txt_persona = ctk.CTkTextbox(
            self.frame_search,
            corner_radius=10,
            height=96,
            font=IOS_FONT_SMALL,
        ...
        self.txt_persona.pack(fill="x", padx=20, pady=(0, 20))
        ...
        ctk.CTkLabel(
            self.frame_msg, text="댓글 가이드(선택)", font=IOS_FONT_MEDIUM, text_color=IOS_COLORS["text_primary"]
        ).pack(anchor="w", padx=20, pady=(20, 14))

        ctk.CTkLabel(
            self.frame_msg, text="AI가 댓글 작성 시 반영할 추가 지시", font=IOS_FONT_SMALL, text_color=IOS_COLORS["text_secondary"]
        ).pack(anchor="w", padx=20, pady=(0, 8))
        ...
        # 독립 스크롤 대상 텍스트박스 등록
        self._scrollable_textboxes = [self.txt_log, self.txt_persona, self.txt_msg]
    ...
    def on_start(self):
        ...
        persona_profile = self.txt_persona.get("1.0", "end").strip()
        if not persona_profile:
            self.log_msg("⚠️ 댓글 페르소나를 입력하세요.")
            return

        gemini_api_key = self.entry_api_key.get().strip()
        if not gemini_api_key:
            self.log_msg("⚠️ Gemini API Key를 설정하세요.")
            return
